...
- **重构基准**：当前仓库（unknowIfGuestInDream/dalian-dagunzi-fx）的部分代码实现与真实游戏规则存在偏差。在重构核心逻辑（如发牌、出牌校验、计分、进贡、升级等）时，必须以本文档提供的规则为唯一事实来源（Source of Truth），若现有代码与此冲突，请覆盖重写现有代码。
