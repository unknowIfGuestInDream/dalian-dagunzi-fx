...
- **AI智能优化**（三个难度级别均已优化）：
  ...
  - 队友赢墩时优先给非特殊主牌的分牌（5/10/K），保留王牌、主花色2和主牌级不浪费
...
- **检查更新**：启动时自动检查新版本（每日一次），也可通过"帮助"菜单手动检查，支持跳转 GitHub Release 下载页面
...
- 王最大，其次主牌级，再次主花色，副花色最小。
- **2 不是常主**，它只是最大的普通牌（2 > A），不具备主牌身份；只有主花色的 2 才算主牌（为最大的主花色普通牌，仅次于主牌级）。
- 同一花色内：2 > A > K > Q > J > 10 > … > 3。
